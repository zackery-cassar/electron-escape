...
export class EscapeRoomClient extends BaseClient {
  ...
  protected callback(topic: string, message: string): void {
    try {
      // Try to parse the message as JSON. If it fails, log an error and silently fail.
      ...
      console.error(
        `Failed to parse MQTT message for topic ${topic} - It's probably not in a JSON format:`,
        error
      )
      return
    }
